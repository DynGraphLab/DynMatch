...
do 
        if [ $COUNT -eq "1" ]; then
            echo "plot \"final_output_time\" using 1:2 with steps lw 2 lt $COUNT title '$line', \\" >> plot_current_time.gnuplot
     else 
            RHS=` echo "$COUNT+1" | bc`
            echo "\"final_output_time\" using $COUNT:$RHS with steps lw 2 lt $COUNT title '$line', \\" >> plot_current_time.gnuplot
